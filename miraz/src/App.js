...
function App() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { professor, branch, section, subjects } = formDetails;

    if (!professor || !branch || !section || subjects.some(subject => !subject.name || !subject.totalLectures)) {
      ...
      {showDashboard ? (
        ...
        <div className="dashboard">
          ...
          {!showAttendancePage && !showSummaryPage ? (
                ...
                {formDetails.subjects.map((subject, index) => (
                  <div key={`subject-${index}`}>
                    ...
                    >
                      ...
                      {options.subjects
                        .filter(subjectOption => !formDetails.subjects.some(s => s.name === subjectOption) || subject.name === subjectOption)
                    ...
                    <input
                      ...
                      onChange={(e) => handleSubjectChange(index, "totalLectures", e.target.value)}
                      required
                    />
                  </div>
                ))}
            ...
            <AttendancePage
              students={students}
              totalClasses={formDetails.subjects.reduce((total, subject) => total + parseInt(subject.totalLectures || 0), 0)}
              onSave={handleSaveAttendance}
              onShowSummary={handleShowSummary}
              subjects={formDetails.subjects}
            />
          ) : (
            <SummaryPage
